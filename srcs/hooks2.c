...
{
	ms->game->pl_pos.x = 3.0;
	ms->game->pl_pos.y = 3.0;
	ms->game->pl_dir.x = -1;
	ms->game->pl_dir.y = 0;
	ms->game->plane.x = 0;
...
{
	ms->game->map.x = (int)ms->game->pl_pos.x;
	ms->game->map.y = (int)ms->game->pl_pos.y;
}

void	set_delta_dist(t_ms *ms)
{
	ms->game->delta_dist.x = fabs(1 / ms->game->map.x);
	ms->game->delta_dist.y = fabs(1 / ms->game->map.y);
}
...
void	set_side_dist(t_ms *ms)
{
	...
	{
		ms->game->step.x = -1;
		ms->game->side_dist.x = (ms->game->pl_pos.x - ms->game->map.x) * ms->game->delta_dist.x;
	}
	else
	{
		ms->game->step.x = 1;
		ms->game->side_dist.x = (ms->game->map.x + 1.0 - ms->game->pl_pos.x) * ms->game->delta_dist.x;
	}
	if(ms->game->ray_dir.y < 0)
	{
		ms->game->step.y = -1;
		ms->game->side_dist.y = (ms->game->pl_pos.y - ms->game->map.y) * ms->game->delta_dist.y;
	}
	else
	{
		ms->game->step.y = 1;
		ms->game->side_dist.y = (ms->game->map.y + 1.0 - ms->game->pl_pos.y) * ms->game->delta_dist.y;
	}
}
...
void	dda(t_ms *ms)
{
	while(1)
	{
		if(ms->game->side_dist.x < ms->game->side_dist.y)
		{
			...
			ms->game->map.x += ms->game->step.x;
			ms->game->side = 0;
		}
		else
		{
			ms->game->side_dist.y += ms->game->delta_dist.y;
			ms->game->map.y += ms->game->step.y;
			ms->game->side = 1;
		}
		if(ms->map[(int)ms->game->map.x][(int)ms->game->map.y])
			break;
	}
	if(ms->game->side == 0)
